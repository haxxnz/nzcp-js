...
test("Bad Public Key pass is unsuccessful", async () => {
  const result = await verifyNZCovidPass(badPublicKeyPass, nzcpExamplesTrustedIssuers);
...
test("Public Key Not Found pass is unsuccessful", async () => {
  const result = await verifyNZCovidPass(publicKeyNotFoundPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Modified Signature pass is unsuccessful", async () => {
  const result = await verifyNZCovidPass(modifiedSignaturePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Modified Payload pass is unsuccessful", async () => {
  const result = await verifyNZCovidPass(modifiedPayloadPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Non base-32 string in the payload Pass is unsuccessful", async () => {
  const result = await verifyNZCovidPass(notBase32, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
