import { verifyPassWithTrustedIssuers } from "./main";
...
test("Valid pass is successful", async () => {
  const result = await verifyPassWithTrustedIssuers(validPass, nzcpExamplesTrustedIssuers);
...
test("Bad Public Key pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(badPublicKeyPass, nzcpExamplesTrustedIssuers);
...
test("Public Key Not Found pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(publicKeyNotFoundPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Modified Signature pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(modifiedSignaturePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Modified Payload pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(modifiedPayloadPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Expired Pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(expiredPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Not Active pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(notActivePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Non base-32 string in the payload Pass is unsuccessful", async () => {
  const result = await verifyPassWithTrustedIssuers(notBase32, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
