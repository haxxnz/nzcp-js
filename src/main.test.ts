...
test("Valid pass is successful", async () => {
  const result = await verifyPass(validPass, nzcpExamplesTrustedIssuers);
...
test("Bad Public Key pass is unsuccessful", async () => {
  const result = await verifyPass(badPublicKeyPass, nzcpExamplesTrustedIssuers);
...
test("Public Key Not Found pass is unsuccessful", async () => {
  const result = await verifyPass(publicKeyNotFoundPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Modified Signature pass is unsuccessful", async () => {
  const result = await verifyPass(modifiedSignaturePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Modified Payload pass is unsuccessful", async () => {
  const result = await verifyPass(modifiedPayloadPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
  expect(result.violates?.section).toBe("3");
});
...
test("Expired Pass is unsuccessful", async () => {
  const result = await verifyPass(expiredPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Not Active pass is unsuccessful", async () => {
  const result = await verifyPass(notActivePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Non base-32 string in the payload Pass is unsuccessful", async () => {
  const result = await verifyPass(notBase32, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
