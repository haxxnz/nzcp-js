...
test("Valid pass is successful", async () => {
  const result = await validateNZCovidPass(validPass, nzcpExamplesTrustedIssuers);
...
>>>>>>> f8251c2889932f0f6a65a1fc4daf063e54e4ddb4
});
...
test("Expired Pass is unsuccessful", async () => {
  const result = await validateNZCovidPass(expiredPass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
...
test("Not Active pass is unsuccessful", async () => {
  const result = await validateNZCovidPass(notActivePass, nzcpExamplesTrustedIssuers);
  expect(result.success).toBe(false);
